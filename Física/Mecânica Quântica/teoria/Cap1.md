...
**Sugestão de imagem 1:** um gráfico comparando a curva de Planck com a curva clássica de Rayleigh‑Jeans, mostrando a divergência ultravioleta. Pesquise por algo como *"blackbody radiation spectrum Planck vs Rayleigh-Jeans ultraviolet catastrophe graph"*. Instruções de como inserir essa imagem via GitHub estão na [Seção 14](#14-apêndice-como-inserir-as-imagens-sugeridas-via-github).

```markdown
![Espectro de radiação de corpo negro: comparação entre a lei de Rayleigh-Jeans (catástrofe do ultravioleta) e a lei de Planck](https://raw.githubusercontent.com/SEU-USUARIO/SEU-REPOSITORIO/main/imagens/corpo-negro-planck.png)
...
```markdown
![Diagrama do efeito fotoelétrico: um fóton de energia hν colide com um elétron na superfície metálica e o ejeta com energia cinética (hν - W₀)](https://raw.githubusercontent.com/SEU-USUARIO/SEU-REPOSITORIO/main/imagens/efeito-fotoeletrico.png)
...
```markdown
![Diagrama do efeito Compton: fóton incidente de comprimento de onda λ colide com elétron em repouso e é espalhado em ângulo θ com comprimento de onda λ' maior](https://raw.githubusercontent.com/SEU-USUARIO/SEU-REPOSITORIO/main/imagens/efeito-compton.png)
...
## 14. Apêndice: como inserir as imagens sugeridas via GitHub

Como arquivos Markdown puros não armazenam imagens dentro de si — eles apenas *referenciam* um endereço (URL) onde a imagem está hospedada — a forma mais simples e gratuita de hospedar as imagens deste documento é usando o próprio GitHub como servidor de arquivos estáticos. Passo a passo:

1. **Crie um repositório no GitHub** (pode ser público ou privado, mas privado exige autenticação para acessar a imagem depois). Por exemplo: `notas-mecanica-quantica`.
2. **Crie uma pasta** dentro do repositório, por exemplo `imagens/`, e faça upload dos arquivos de imagem (PNG ou JPG) que você baixou ou criou para ilustrar cada seção (corpo negro, efeito fotoelétrico, efeito Compton).
3. Depois do upload, **clique no arquivo de imagem** dentro da interface do GitHub e depois no botão **"Raw"** (ou "Download raw file"). A URL que aparece na barra de endereço terá este formato:

   ```
   https://raw.githubusercontent.com/SEU-USUARIO/SEU-REPOSITORIO/main/imagens/nome-do-arquivo.png
   ```

4. **Substitua os placeholders** `SEU-USUARIO`, `SEU-REPOSITORIO` e `nome-do-arquivo.png` nos blocos de código de cada figura deste documento (Figuras 1, 2 e 3) por essa URL real.
5. A sintaxe Markdown para exibir a imagem é:

   ```markdown
   ![Texto alternativo descrevendo a imagem](URL-DA-IMAGEM-RAW)
   ```

   O texto entre `[ ]` é o texto alternativo (importante para acessibilidade e para o caso de a imagem não carregar); a URL entre `( )` é o link "raw" obtido no passo 3.

> **Observação importante sobre direitos de imagem:** ao escolher imagens encontradas em buscas na internet, prefira fontes com licença aberta (Wikimedia Commons, domínio público, Creative Commons) ou produza seus próprios gráficos (por exemplo, com Python/Matplotlib, plotando a lei de Planck e a lei de Rayleigh-Jeans você mesmo — isso, aliás, é um excelente exercício complementar de física computacional). Evite reproduzir, sem licença, imagens tiradas diretamente de livros didáticos comerciais.
